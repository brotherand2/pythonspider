...
def ContentSave(item):
    ...
    save_content=[source,createdtime.strftime("%Y-%m-%d %H:%M:%S"),content ,type,district ,property ,centralbank ]
    #print (save_content)
    #pprint (save_content)
    #ResultSave(save_host, save_port, save_name, save_password, save_database, save_collection, save_content)

    # 打开数据库连接
    db = MySQLdb.connect("localhost", "root", "123456", "wallstreet",charset='utf8')
    ...
    # 创建数据表SQL语句
    sql = """create table  if not exists wallstreet  (
             SOURCE  TEXT DEFAULT '',
             ...
             centralbank TEXT DEFAULT '') DEFAULT CHARSET=utf8"""

    cursor.execute(sql)

    try:
        # 执行sql语句
        cursor.execute("INSERT INTO wallstreet2 VALUES \
           (%s, %s, %s, %s, %s , %s, %s)" ,
        save_content)
        # 提交到数据库执行
        db.commit()
